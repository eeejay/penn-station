...
function getPhoneNumber(client, phone_number) {
  return new Promise((resolve, reject) => {
    client.incomingPhoneNumbers.list({ phoneNumber: phone_number }, (err, result) => {
      if (!result.incomingPhoneNumbers.length) {
        reject("Phone number: '" + phone_number + "' is not configured in Twilio.");
        return;
      }
...
function configurePhoneNumber(client, number_details, app) {
  return new Promise((resolve, reject) => {
    if (number_details.voiceApplicationSid == app.sid &&
      ...
      client.incomingPhoneNumbers(number_details.sid).update(
        { smsApplicationSid: app.sid,
          voiceApplicationSid: app.sid }, (err, result) => resolve());;
    }
  });
}
...
router.all('/voice-twiml', function(req, res, next) {
  ...
  if (req.body.number) {
    twiml.dial(req.body.number, {callerId: req.body.callerId});
  } else {
    twiml.dial({callerId: req.body.From}, function(node) {
